...
def render(win, game_state: GameState, round_state: Optional[RoundState], dirty_rects: Optional[List[Tuple[int, int]]] = None) -> None:
    ...
    for pos_x, pos_y in old_projectile_positions - current_projectile_positions:
        if (pos_x, pos_y) not in drawn_cells and 0 <= pos_x < visible_cols and 0 <= pos_y < visible_rows:
            ...
            if ((pos_x, pos_y) not in tower_positions and 
                (round_state is None or (pos_x, pos_y) != round_state.agent_pos)):
    ...
    old_agent_pos = (-1, -1)
    if round_state and hasattr(round_state, 'old_agent_pos'):
        old_agent_pos = round_state.old_agent_pos
    ...
    if round_state:
        ...
        if 0 <= agent_x < visible_cols and 0 <= agent_y < visible_rows:
            ...
            health_percentage = round_state.agent_health / 100.0  # Assuming max health is 100
        ...
        if old_agent_pos != round_state.agent_pos and old_agent_pos != (-1, -1):
            old_x, old_y = old_agent_pos
        ...
        round_state.old_agent_pos = round_state.agent_pos
